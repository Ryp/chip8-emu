...
namespace chip8
{
    void load_program(CPUState& state, const u8* program, u16 size)
    {
        Assert((size & 0x0001) == 0); // Unaligned size
        Assert(is_valid_memory_range(MinProgramAddress, size, MemoryUsage::Write));
